...
main(int argc, char **argv)
{
  ...
  default:
    vanet_throw_logic_error("Invalid test found: "
        << options_map["test-case"].as<TestCase> () << ".");
  ...
  for (; vertex_it != vertex_end_it; ++vertex_it)
    {
      ...
      if (parents_range.empty())
        {
          make_vertex_prior(bn[*vertex_it].random_variable,
              parents_range.begin(), prior.at(*vertex_it), prior_count);
        }
      else
        {
...
test_alarm_net()
{
  ...
  std::pair<DiscreteBayesianNetwork::vertex_iterator,
      DiscreteBayesianNetwork::vertex_iterator> vertex_range = vertices(
      alarmNetwork);
  for (DiscreteBayesianNetwork::vertex_iterator it = vertex_range.first; it
      != vertex_range.second; ++it)
    {
