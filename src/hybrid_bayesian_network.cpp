...
namespace vanet
{
  ...
  public:
    ...
    {
      x_ = dynamic_cast<DiscreteRandomVariable*> (&X_v->random_variable());
      if (x_ == 0)
      ...
      try
        {
          ...
          RandomConditionalProbabilities
              & x =
                  dynamic_cast<RandomConditionalProbabilities&> (X_v_->random_variable());
          variate_generator<RandomNumberEngine&, CondDirichletDistribution>
              sampling_variate_(random_number_engine, sample_distribution);
          x = sampling_variate_();
        }
      catch (const bad_cast& bc)
        {
          vanet_throw_network_error("DirichletMarkovBlanket: Can only process "
              "random variables of type RandomConditionalProbabilities. "
              << bc.what());
        }
      catch (const boost::bad_get& bg)
        {
          vanet_throw_network_error(
              "DirichletMarkovBlanket: Cannot use distribution of type "
      ...
      iterator params_vertex)
  {
    ...
    RandomProbabilities * params =
        dynamic_cast<RandomProbabilities*> (params_rv);
    if (!params)
      throw runtime_error(
      ...
      const VertexReferences& condition_vertices)
  {
    ...
    for (VertexReferences::const_iterator c_it = condition_vertices.begin(); c_it
        != condition_vertices.end(); ++c_it)
      {
      ...
      unsigned int burn_in_iterations, unsigned int collect_iterations)
  {
    ...
    DiscreteRandomVariable* x =
        dynamic_cast<DiscreteRandomVariable*> (&X_it->random_variable());
    if (x == 0)
      throw runtime_error(
    ...
    for (iterator v = vertices_.begin(); v != vertices_.end(); ++v)
      {
        ...
        else if (get<ConditionalCategoricalDistribution> (&v->distribution())
            != 0)
          non_evidence_vertices.push_back(
              make_pair(v, new CategoricalMarkovBlanket(v)));
    ...
    for (vertex_it = vertices_begin_it; vertex_it != vertices_end_it; ++vertex_it)
      {
    ...
    // Burn in sampling
    for (unsigned int iteration = 0; iteration < burn_in_iterations; iteration++)
      {
        for (vertex_it = vertices_begin_it; vertex_it != vertices_end_it; ++vertex_it)
          {
    ...
    for (unsigned int iteration = 0; iteration < collect_iterations; iteration++)
      {
        for (vertex_it = vertices_begin_it; vertex_it != vertices_end_it; ++vertex_it)
          {
            vertex_it->second->sample();
