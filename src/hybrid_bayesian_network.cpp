...
HybridBayesianNetwork::iterator
HybridBayesianNetwork::add_conditional_categorical(
const DiscreteRandomVariable& var, const VertexReferences& condition_vertices,
iterator params_vertex)
{
 // First, convert everything that could fail.
RandomVariable* params_rv = &params_vertex->random_variable();
...
{
RandomConditionalProbabilities* new_var = new RandomConditionalProbabilities(
var);
...
non_evidence_vertices.push_back(make_pair(v, new CategoricalMarkovBlanket(v)));
...
{
vertex_it->first->random_variable().assign_random_value(random_number_engine);
}
