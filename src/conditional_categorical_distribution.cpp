...
namespace vanet
{
  ...
  ConditionalCategoricalDistribution::ConditionalCategoricalDistribution(
      ...
      const std::vector<const DiscreteRandomVariable*>& condition_vars) :
        cpt_(
            *new RandomConditionalProbabilities(var,
                make_condition(condition_vars))),
        condition_vars_(condition_vars), self_allocated_params_(true),
        var_(var)
  {
  ...
  ConditionalCategoricalDistribution::ConditionalCategoricalDistribution(
    ...
    cpt_(params), condition_vars_(condition_vars),
        self_allocated_params_(false), var_(var)
  {
  }

  ConditionalCategoricalDistribution::ConditionalCategoricalDistribution(
      const ConditionalCategoricalDistribution& ccd) :
    cpt_(make_parameters_from_other_ccd(ccd)),
        condition_vars_(ccd.condition_vars_),
        self_allocated_params_(ccd.self_allocated_params_), var_(ccd.var_)
  {
      ...
      const ConditionReferences& references)
  {
    ...
    for (ConditionReferences::const_iterator p_it = references.begin(); p_it
        != references.end(); ++p_it)
      {
