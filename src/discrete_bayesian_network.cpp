  ...
  if (vp.value_is_evidence)
  ...
  else
    {
      ...
      for (x = X_value_range.begin(); x != X_value_range.end(); ++x)
        ...
        probability_sum += conditional_probability
            * enumerate_all(bn, begin + 1, end);
      }
...
for (unsigned int iteration = 0; iteration < iterations; ++iteration)
  {
  for (DiscreteBayesianNetwork::vertex_iterator vertex_it = vertex_range.first;
      vertex_it != vertex_range.second; ++vertex_it)
        {
...
else
{
  ...
  prob_dis.set(vertex_var, parents_var, value_count / parents_count);
}
}
}
...
throw runtime_error(
    "learn_ml: Could not find a value for the variable " + var_name
        + " in the data.");
...
pair<DiscreteBayesianNetwork::in_edge_iterator,
  DiscreteBayesianNetwork::in_edge_iterator> in_edge_range = in_edges(
  *vertex_it, bn);
for (DiscreteBayesianNetwork::in_edge_iterator inEIt = in_edge_range.first;
  inEIt != in_edge_range.second; ++inEIt)
    {
DiscreteBayesianNetwork::vertex_descriptor parent_v = source(*inEIt, bn);
const string& parent_name = bn[parent_v].random_variable.name();
Variables::const_iterator parent_it = variables.find(parent_name);
if (parent_it == variables.end())
  throw runtime_error(
      "learn_ml: Could not find a value for the variable " + parent_name
          + " in the data.");
 ...
 */
for (DiscreteBayesianNetwork::vertex_iterator v_it = vertex_range.first;
v_it != vertex_range.second; ++v_it)
{
...
DiscreteRandomVariable::Range parents_range =
parents_variable(bn, *v_it).value_range();
if (parents_range.empty())
{
learn_multinomial_fill_cpd(vertex_var, parents_range.begin(), value_count_dis,
parents_count_dis, prob_dis);
}
else
{
...
{
learn_multinomial_fill_cpd(vertex_var, parents_var, value_count_dis,
parents_count_dis, prob_dis);
}
}
}
...
vector<ConditionalCountDistribution>::const_iterator prior_it = prior.begin();
vector<CountDistribution>::iterator parents_it = parents_counts.begin();
for (; prior_it != prior.end(); ++prior_it, ++parents_it)
{
for (ConditionalCountDistribution::const_iterator counts_it = prior_it->begin();
counts_it != prior_it->end(); ++counts_it)
{
...
pair<DiscreteBayesianNetwork::vertex_iterator,
DiscreteBayesianNetwork::vertex_iterator> vertices_range = boost::vertices(bn);
for (DiscreteBayesianNetwork::vertex_iterator vIt = vertices_range.first;
vIt != vertices_range.second; ++vIt)
{
...
os << " of variable " << vp.random_variable.name() << " with no evidence\n";

for_each(vp.distribution.begin(), vp.distribution.end(), StreamOut(os, "    "));
}
...
for (DiscreteBayesianNetwork::edge_iterator eIt = edges_range.first;
eIt != edges_range.second; ++eIt)
{
os << "  Edge " << *eIt << "\n";
}
os << endl;
...
{
DiscreteBayesianNetwork::vertex_descriptor parent = source(*edge_it, bn);
...
for (; edge_it != edge_end_it; ++edge_it)
{
DiscreteBayesianNetwork::vertex_descriptor child_v = target(*edge_it, bn);
...
std::tr1::variate_generator<_RandomNumberGenerator&, CategoricalDistribution> sampling_variate(
rng, sampling_distribution);
x = sampling_variate();
}
